...
class LLMTaskNode(Node):
    def __init__(self):
        ...
        # self.system_prompt = (
        #     "You are a robotics task planner for a 6-DOF manipulator with a gripper.\n"
        #     "You receive natural-language commands and must output a pure JSON plan using the available tools below.\n\n"
        #     "=== Available tools ===\n"
        #     "1) detect_object:\n"
        #     "   Detects an object by color using the camera.\n"
        #     "   params: { \"color\": <string> }\n\n"
        #     "2) move_tcp:\n"
        #     "   Moves the robot TCP (tool center point) to a target pose.\n"
        #     "   params: {\n"
        #     "       \"frame\": \"link_base\",\n"
        #     "       \"position\": {\"x\": <float>, \"y\": <float>, \"z\": <float>},\n"
        #     "       \"orientation_rpy\": {\"roll\": <float>, \"pitch\": <float>, \"yaw\": <float>} OR null\n"
        #     "   }\n"
        #     "   If orientation_rpy is null, keep the current TCP orientation.\n"
        #     "   The LLM can also modify only some angles (e.g. 'keep same yaw, set pitch -1.57').\n\n"
        #     "3) pick_place_vision:\n"
        #     "   Performs a complete pick and place using vision.\n"
        #     "   params: {\n"
        #     "       \"color\": <string>,\n"
        #     "       \"place_position\": {\"x\": <float>, \"y\": <float>, \"z\": <float>},\n"
        #     "       \"pick_orientation_rpy\": {\"roll\": <float>, \"pitch\": <float>, \"yaw\": <float>} OR null,\n"
        #     "       \"place_orientation_rpy\": {\"roll\": <float>, \"pitch\": <float>, \"yaw\": <float>} OR null,\n"
        #     "       \"lift_offset\": <float> (optional, meters to lift above object before or after pick)\n"
        #     "   }\n"
        #     "   If no orientation is provided, the robot keeps its current orientation.\n\n"
        #     "4) gripper_control:\n"
        #     "   Opens or closes the gripper.\n"
        #     "   params: { \"state\": \"open\" | \"close\" }\n\n"
        #     "Output format (only valid JSON, no extra text):\n"
        #     "{\n"
        #     "  \"actions\": [\n"
        #     "    {\"tool\": \"move_tcp\", \"params\": {...}},\n"
        #     "    {\"tool\": \"gripper_control\", \"params\": {\"state\": \"close\"}},\n"
        #     "    {\"tool\": \"pick_place_vision\", \"params\": {...}}\n"
        #     "  ]\n"
        #     "}\n"
        #     "Rules:\n"
        #     "- Always output JSON only.\n"
        #     "- Use SI units (meters, radians).\n"
        #     "- When the user specifies only some orientation values, fill in the others as null (meaning 'keep same').\n"
        #     "- If the camera is top-down, remember that depth may be underestimated; add a 0.05 m safety margin in z when necessary.\n"
        # )
    ...
    def handle_llm_task(self, user_cmd: str):
        try:
            ...
            # 🧹 Rimuovi i backtick e tag "json" se presenti
            ...
            # 🧩 TROVA SOLO IL BLOCCO JSON VALIDO (tra primo { e ultimo })
            import re
    ...
    def execute_pick_place_vision(self, params: dict):
        ...
        # ⚠️ Convenzione: se l'LLM NON specifica le RPY, lasciamo 0.0
        # Il nodo pick_place_vision_llm interpreterà 0,0,0 come "usa default"
    ...
    def execute_gripper_control(self, params: dict):
        """
        Esegue l'apertura o chiusura del gripper in base a 'state': 'open' o 'close'.
        """
        ...
        # Crea un action client locale per sicurezza (non blocca il resto del nodo)
