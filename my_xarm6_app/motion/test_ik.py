...
class TestIKClient(Node):
    ...
    def __init__(self):
        ...
        # Nome del service MoveIt per IK (di default)
    ...
    def send_ik_request(self):
        ...
        # Nome del planning group come definito nel tuo MoveIt setup
        req.ik_request.group_name = 'xarm6'

        # Nome del link TCP (end-effector) come da URDF/SRDF
        req.ik_request.ik_link_name = 'link_tcp'

        # Pose target nel frame di riferimento desiderato
        pose = PoseStamped()
        pose.header.frame_id = 'link_base'
        pose.header.stamp = self.get_clock().now().to_msg()

        # Orientation: da RPY a quaternion (come nel tuo esempio)
        ...
        # Opzionale: usa lo stato di robot "vuoto" (MoveIt userà lo stato corrente)
        req.ik_request.avoid_collisions = True

        # Chiamata asincrona al service
    ...
    def process_result(self, res: GetPositionIK.Response):
        if res.error_code.val == res.error_code.SUCCESS:
            ...
            # Stampa coppie joint-name / joint-position
